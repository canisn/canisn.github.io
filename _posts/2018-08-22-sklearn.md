# 1. k-means, min_batch_k-means

k-means是一种非监督聚类算法，很常用。但是也有一个严重的问题，当数据集较大的时候，算法的时间和空间复杂度都呈现出爆炸性增长，特别是对于大数据领域动辄上十万百万级数据样本，如果数据的维度在相应的提高时，通常的k-means算法就很费劲了。于是min_batch_k-means挺身而出，弥补了这一项不足。

虽然在最终的效果上看min_batch_k-means的结果与k-means有细微的差别，但是在实际工程应用中这一点误差完全可以忽略不计。

min_batch_k-means在sklearn中已经封装好实现了，相当好用。此处diss TensorFlow，以下展示一个两个算法的对比用例。

先上测试结果：

![测试图片](https://raw.githubusercontent.com/canisn/canisn.github.io/master/_src/min_k_means.png)
